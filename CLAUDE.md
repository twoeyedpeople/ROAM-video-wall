...
- **The screen must never go black, and every step has an exit.** A film that stalls for `FILM_STALL_MS`, fails to decode, or is missing from the cache is skipped and marked played (sent to the back, so it cannot hold the wall).
- **Only a decode failure or a missing copy discards the local copy, once per film version per page load; a stall never does.** On 2026-09-16 plays kept failing overnight, each failure discarded and re-fetched a 43 MB film, and the wall pulled about 1,000 films (41.8 GB of booth transfer) in 12 hours. A stall is the browser, not the file. The watchdog also ignores time the page is hidden, and a failed download backs off exponentially (`DOWNLOAD_RETRY_MS` doubling to `DOWNLOAD_RETRY_MAX_MS`), because a failure after the last range costs the whole film. The campaign spot has the same three exits, and hands the step to the drawn card rather than to the next guest, because a card is the one thing here with no file to lose. `INTERSTITIAL_MAX_MS` still covers an interstitial that somehow never ends either way.
...
- **The countdown's digits are pure CSS** (`count-in` with staggered `animationDelay`), and so is the title-to-count cross-fade. The loop owns the one timer that matters, the move to the film at `COUNTDOWN_TOTAL_MS`. `COUNTDOWN_STEP_MS` and both the `count-in` and `sweep` durations in `tailwind.config.ts` must agree: the leader's sweep is one turn per digit. The sweep starts with the digits, and the cut to black on the 1 is computed from the hand's angle (`HAND_DEG` in `CountdownDial.tsx`), so the black lands when the hand is straight down wherever the hand is drawn.
...
- **The Up Next panel's "MINS AWAY" is arithmetic, not data.** Nothing on the feed carries a length. `machine/eta.ts` builds the figure from the loop's own beats and the film's duration once the file reports it, and `useWallLoop` re-reads it every second but stores it only when the whole minute the panel shows changes (`etaMinutes`, which the copy uses too), so the figure steps down on the minute without re-rendering the wall every second. It used to store only on a half-minute move against the last stored value, read every 5 s, which held a figure well past its minute and read as stuck. It counts down to the next finished film on the wall, never to a film still rendering: those are not on the feed.
